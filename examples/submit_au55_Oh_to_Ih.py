...
parent_atoms = ase.io.read(
    '/mnt/bulk-data/Dropbox/BNL_Project/Simulations/Models.d/2-AuNP-DFT.d/SizeVariation.d/Au55.Ih_ini.xyz'
)
...
calc1 = insert_calc('Au Ih Target Rw', 'PDF', calc_kwargs1, calc_exp=pdf)

calc_kwargs2 = {'k': 100, 'rt': exp_dict['rmin']}
calc2 = insert_calc('Au Ih Target spring', 'Spring', calc_kwargs2)

# Create the combined Potential Energy Surface (PES)
calc_list = [calc1, calc2]
pes = insert_pes('Au Ih Target Rw Spring', calc_list)

params, = find_simulation_parameter_document(name='T=1, iter=100, accept=.65')

starting_atoms = ase.io.read('/mnt/bulk-data/Dropbox/BNL_Project/Simulations/Models.d/2-AuNP-DFT.d/SizeVariation.d/Au55.Oh_ini.xyz')

# Add the atoms to the DB
start_config = insert_atom_document('Au Oh Starting', starting_atoms)
# Finally create the simulation
sim = insert_simulation('Au Oh -> Ih', params, start_config, pes)
